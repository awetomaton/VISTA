...
from vista.algorithms.imagery.prf_photometry import (
    estimate_detector_flux_counts,
    summarize_flux_counts,
)
...
class DetectionsPanel(QWidget):
    ...
    def __init__(self, viewer):
        ...
        self.waiting_for_track_selection = (
            False  # Flag when waiting for user to select track
        )
        self.init_ui()
        ...
        self.undo_stack.undo_description_changed.connect(
    ...
    def init_ui(self):
        ...
        self.bulk_property_combo.currentIndexChanged.connect(
            self.on_bulk_property_changed
        )
        ...
        self.export_detections_btn.setEnabled(
            False
        )  # Disabled until detectors selected
        ...
        self.delete_selected_detections_btn.setEnabled(
            False
        )  # Disabled until detectors selected
        self.delete_selected_detections_btn.clicked.connect(
        ...
        self.estimate_prf_flux_btn.clicked.connect(
            self.estimate_prf_flux_for_selected_detectors
        )
        ...
        self.merge_detections_btn.setEnabled(
            False
        )  # Disabled until 2+ detectors selected
        self.merge_detections_btn.clicked.connect(self.merge_detections)
        self.merge_detections_btn.setToolTip(
            "Merge selected detectors into a single detector"
        )
        ...
        self.copy_to_sensor_btn.setToolTip(
        ...
        self.create_track_from_detections_btn.clicked.connect(
            self.create_track_from_selected_detections
        )
        self.create_track_from_detections_btn.setEnabled(False)
        self.create_track_from_detections_btn.setToolTip(
            "Create a track from the selected detections"
        )
        ...
        self.edit_detector_btn.setEnabled(
            False
        )  # Disabled until single detector selected
        ...
        self.delete_selected_points_btn.setEnabled(
            False
        )  # Disabled until detections are selected
        self.delete_selected_points_btn.clicked.connect(
            self.delete_selected_detection_points
        )
        self.delete_selected_points_btn.setToolTip(
            "Delete selected detection points from their detectors"
        )
        ...
        self.label_detections_btn.setEnabled(
            False
        )  # Disabled until detections are selected
        self.label_detections_btn.setToolTip(
            "Set labels on selected detection points (replaces existing labels)"
        )
        ...
        self.hidden_columns_label.setStyleSheet(
        ...
        self.detections_table.setSelectionBehavior(
            QTableWidget.SelectionBehavior.SelectRows
        )
        self.detections_table.setSelectionMode(
            QTableWidget.SelectionMode.ExtendedSelection
        )

        # Connect selection changed signal to update Edit Detector button state
        self.detections_table.itemSelectionChanged.connect(
        ...
        )  # Labels (can have multiple labels)
        header.setSectionResizeMode(
            3, QHeaderView.ResizeMode.ResizeToContents
        )  # Color (fixed)
        # header.setSectionResizeMode(4, QHeaderView.ResizeMode.ResizeToContents)  # Marker (dropdown)
        self.detections_table.setColumnWidth(
            4, 80
        )  # Set reasonably large width to accomodate delegate
        header.setSectionResizeMode(
        ...
        self.detections_table.setItemDelegateForColumn(
            2, self.detections_labels_delegate
        )  # Labels

        self.detections_color_delegate = ColorDelegate(self.detections_table)
        self.detections_table.setItemDelegateForColumn(
            3, self.detections_color_delegate
        )  # Color

        self.detections_marker_delegate = MarkerDelegate(self.detections_table)
        self.detections_table.setItemDelegateForColumn(
            4, self.detections_marker_delegate
        )  # Marker

        self.detections_line_thickness_delegate = LineThicknessDelegate(
            self.detections_table
        )
        self.detections_table.setItemDelegateForColumn(
            6, self.detections_line_thickness_delegate
        )  # Line thickness
        ...
        self.detections_table.horizontalHeader().setContextMenuPolicy(
        ...
        self.detections_table.horizontalHeader().setDragDropMode(
            QHeaderView.DragDropMode.InternalMove
        )
        self.detections_table.horizontalHeader().sectionMoved.connect(
            self.on_detection_column_moved
        )
    ...
    def refresh_detections_table(self):
        ...
        try:
            ...
            if selected_sensor is not None:
                filtered_detectors = [
                    det
                    for det in self.viewer.detectors
                    if det.sensor == selected_sensor
                ]
            else:
            ...
            for row, detector in enumerate(filtered_detectors):
                try:
                    ...
                    name_item.setData(
                        Qt.ItemDataRole.UserRole, detector.uuid
                    )  # Store detector UUID
                    ...
                    labels_text = (
                        ", ".join(sorted(unique_labels)) if unique_labels else ""
                    )
                    labels_item = QTableWidgetItem(labels_text)
                    labels_item.setFlags(
                    ...
                    if not color.isValid():
                        print(
                            f"Warning: Invalid color '{detector.color}' for detector '{detector.name}', using red"
                        )
                        color = QColor("red")
                    color_item.setBackground(QBrush(color))
                    color_item.setData(
                        Qt.ItemDataRole.UserRole, detector.color
                    )  # Store original color string
                    self.detections_table.setItem(row, 3, color_item)

                    # Marker
                    self.detections_table.setItem(
                        row, 4, QTableWidgetItem(str(detector.marker))
                    )
                    ...
                    print(
                        f"Error adding detector '{detector.name}' to table at row {row}: {e}"
                    )
                    traceback.print_exc()
    ...
    def _update_detections_header_icons(self):
        ...
        for col_idx in range(len(base_names)):
            ...
            self.detections_table.setHorizontalHeaderItem(
                col_idx, QTableWidgetItem(label)
            )
                    ...
                    if filter_type == "set":
                        ...
                        for i in range(len(detector.frames)):
                            if i < len(detector.labels):
                                ...
                                detection_matches = (
                                    has_no_labels and no_labels_selected
                                ) or (
                                    not has_no_labels
                                    and len(label_filter_values) > 0
                                    and detection_labels.intersection(
                                        label_filter_values
                                    )
                                )
    ...
    def on_detections_header_context_menu(self, pos):
        ...
        if column == 2:
            ...
            filter_action.triggered.connect(
                lambda: self.show_detection_filter_dialog(column)
            )
            menu.addAction(filter_action)

            clear_filter_action = QAction("Clear Filter", self)
            clear_filter_action.triggered.connect(
                lambda: self.clear_detection_column_filter(column)
            )
            clear_filter_action.setEnabled(
                column in self.detection_column_filters
                and bool(self.detection_column_filters[column])
            )
        ...
        for col_idx in range(len(column_names)):
            ...
            action.triggered.connect(
                lambda checked, col=col_idx: self.toggle_detection_column_visibility(
                    col, checked
                )
            )
    ...
    def show_detection_filter_dialog(self, column):
        ...
        current_values = (
            current_filter.get("values", set()) if current_filter else set()
        )
        ...
        if dialog.exec() == QDialog.DialogCode.Accepted:
            # Apply filter
            selected_values = {
                value for value, cb in checkboxes.items() if cb.isChecked()
            }
    ...
    def save_detection_column_visibility(self):
        ...
        for col_idx in range(1, 8):
            ...
            self.settings.setValue(
                key, self.detection_column_visibility.get(col_idx, True)
            )
    ...
    def save_detection_column_order(self):
        ...
        header = self.detections_table.horizontalHeader()
        order = [
            header.logicalIndex(visual_idx) for visual_idx in range(header.count())
        ]
        self.settings.setValue("detection_column_order", order)

    def on_detection_column_moved(
        self, logical_index, old_visual_index, new_visual_index
    ):
    ...
    def get_filtered_detection_mask(self, detector):
        ...
        if not isinstance(detector.labels, list):
            ...
            detector.labels.extend(
                [set() for _ in range(len(detector.frames) - len(detector.labels))]
            )
        ...
        if no_labels_selected and not label_filter_values:
            ...
            mask = np.array(
                [len(detector.labels[i]) == 0 for i in range(num_detections)],
                dtype=bool,
            )
            ...
            mask = np.array(
                [
                    len(detector.labels[i]) > 0
                    and bool(detector.labels[i] & label_filter_values)
            ...
            mask = np.array(
                [
                    len(detector.labels[i]) == 0
                    or bool(detector.labels[i] & label_filter_values)
                    for i in range(num_detections)
                ],
                dtype=bool,
            )
    ...
    def apply_bulk_action(self):
        ...
        selected_rows = sorted(
            set(index.row() for index in self.detections_table.selectedIndexes())
        )
        ...
        self.save_undo_state(
            f"Bulk change {property_name} on {len(selected_rows)} detectors"
        )
    ...
    def toggle_selected_detections_visibility(self):
        ...
        selected_rows = set(
            index.row() for index in self.detections_table.selectedIndexes()
        )
    ...
    def delete_selected_detections(self):
        ...
        self.viewer.detectors = [
            d for d in self.viewer.detectors if d.uuid not in detectors_to_delete_uuids
        ]
    ...
    def on_detections_rows_moved(self, source_rows, target_row):
        ...
        new_order = [
            uuid for uuid in displayed_detector_uuids if uuid not in moved_uuids
        ]
        ...
        uuid_to_detector = {
            detector.uuid: detector for detector in self.viewer.detectors
        }
        ...
        non_displayed_detectors = [
            d for d in self.viewer.detectors if d.uuid not in displayed_uuid_set
        ]
        reordered_displayed_detectors = [
            uuid_to_detector[uuid] for uuid in new_order if uuid in uuid_to_detector
        ]
    ...
    def estimate_prf_flux_for_selected_detectors(self):
        ...
        if not selected_rows:
            QMessageBox.warning(
                self, "No Selection", "Please select one or more detectors."
            )
            return
        ...
        for row in selected_rows:
            ...
            detector_uuid = name_item.data(Qt.ItemDataRole.UserRole)
            detector = next(
                (d for d in self.viewer.detectors if d.uuid == detector_uuid), None
            )
        ...
        if not selected_detectors:
            QMessageBox.warning(
                self, "No Detections", "Could not find the selected detectors."
            )
            return

        missing_prf = [
            detector.sensor.name
            for detector in selected_detectors
        ...
        self.save_undo_state(
            f"Estimate PRF flux for {len(selected_detectors)} detector(s)"
        )
        ...
        for detector in selected_detectors:
            ...
            if not imageries:
                ...
                detector.flux_background_counts = np.full(
                    len(detector), np.nan, dtype=np.float64
                )
                detector.flux_residual_ratio = np.full(
                    len(detector), np.nan, dtype=np.float64
                )
                detector.flux_status = ["rejected:no_loaded_imagery_for_sensor"] * len(
                    detector
                )
            ...
            detector.flux_snr = np.asarray(
                [result.snr for result in results], dtype=np.float64
            )
            ...
            low_confidence = sum(
                result.status.startswith("low_confidence") for result in results
            )
            ...
            if summary["count"] == 0:
                if status_counts:
                    ...
                    summaries.append(
                        f"{detector.name}: 0/{len(detector)} estimated; all rejected"
                    )
            else:
                rejected_reasons = {
                    status: count
                ...
                if rejected_reasons:
                    reasons = ", ".join(
                        f"{status} ({count})"
                        for status, count in sorted(
                            rejected_reasons.items(), key=lambda item: item[0]
                        )
                    )
    ...
    def on_edit_detector_clicked(self, checked):
        ...
        if checked:
            ...
            if hasattr(main_window, "deactivate_all_interactive_modes"):
                main_window.deactivate_all_interactive_modes(
                    except_action="edit_detector"
                )
            ...
            if edited_detector:
                ...
                if hasattr(self.parent(), "parent"):
                    ...
                    if hasattr(main_window, "statusBar"):
                        main_window.statusBar().showMessage(
                            "Detector editing cancelled", 3000
                        )
    ...
    def export_detections(self):
        ...
        if not selected_rows:
            QMessageBox.warning(
                self, "No Selection", "Please select one or more detectors to export."
            )
            return
        ...
        if file_path:
            self.settings.setValue(
                "last_detections_export_dir", str(pathlib.Path(file_path).parent)
            )
            try:
                ...
                for detector in selected_detectors:
                    ...
                    all_detections_df = pd.concat(
                        [all_detections_df, detector_df], ignore_index=True
                    )
                ...
                QMessageBox.critical(
                    self, "Export Error", f"Failed to export detections:\n{str(e)}"
                )
    ...
    def copy_to_sensor(self):
        ...
        filtered_only_checkbox.setEnabled(
            bool(self.detection_column_filters)
        )  # Only enable if filters are active
        if self.detection_column_filters:
            filtered_only_checkbox.setToolTip(
                "Only copy detections that match the current label filter"
            )
        else:
    ...
    def label_selected_detections(self):
        ...
        if dialog.exec() == QDialog.DialogCode.Accepted:
            ...
            if self.detection_column_filters and hasattr(
                self.viewer, "update_detection_display"
            ):
                self.viewer.update_detection_display()
    ...
    def delete_selected_detection_points(self):
        ...
        for detector_uuid, data in detector_indices.items():
            ...
            if detector.flux_status:
                detector.flux_status = [
                    status
                    for i, status in enumerate(detector.flux_status)
            ...
            if detector.labels and len(detector.labels) > 0:
                detector.labels = [
                    label for i, label in enumerate(detector.labels) if keep_mask[i]
                ]
            if detector.label_times and len(detector.label_times) > 0:
                detector.label_times = [
                    t for i, t in enumerate(detector.label_times) if keep_mask[i]
                ]
            if detector.labelers and len(detector.labelers) > 0:
                detector.labelers = [
                    name for i, name in enumerate(detector.labelers) if keep_mask[i]
                ]
        ...
        QMessageBox.information(
            ...
            + (
                f"\n{len(detectors_to_remove)} empty detector(s) were also removed."
                if detectors_to_remove
                else ""
            ),
            QMessageBox.StandardButton.Ok,
        )
    ...
    def create_track_from_selected_detections(self):
        ...
        if len(self.selected_detections) < 2:
            QMessageBox.warning(
                ...
                "Select at least 2 detections to create a track.",
            )
            return
        ...
        for detector, frame, index in self.selected_detections:
            ...
            for attr in _FLUX_ARRAY_ATTRS:
                flux_values[attr].append(
                    self._optional_detection_value(detector, attr, index)
                )
    ...
    def start_add_to_existing_track(self):
        ...
        if len(self.selected_detections) < 1:
            QMessageBox.warning(
                self, "No Detections", "Select at least 1 detection to add to a track."
            )
            return
        ...
        main_window = self.window()
        if (
            hasattr(main_window, "lasso_select_action")
            and main_window.lasso_select_action.isChecked()
        ):
        ...
        self.add_to_existing_track_btn.clicked.connect(
        ...
        if hasattr(main_window, "statusBar"):
            main_window.statusBar().showMessage(
                "Click on a track in the viewer to add selected detections to it", 0
            )
    ...
    def on_track_selected_for_adding_detections(self, track):
        ...
        if reply == QMessageBox.StandardButton.Yes:
            ...
            flux_values = {
                attr: list(getattr(track, attr))
                if getattr(track, attr, None) is not None
                else [np.nan] * len(track.frames)
                for attr in _FLUX_ARRAY_ATTRS
            }
            flux_status = (
                list(track.flux_status)
                if track.flux_status
                else [""] * len(track.frames)
            )
            ...
            sorted_flux_status = [
                status
                for i, status in enumerate(sorted_flux_status)
    ...
    def merge_detections(self):
        ...
        if len(selected_rows) < 2:
            QMessageBox.warning(
                self, "Cannot Merge", "Please select at least 2 detectors to merge."
            )
            return
        ...
        if len(detectors_to_merge) < 2:
            QMessageBox.warning(
                self, "Cannot Merge", "Could not find enough valid detectors to merge."
            )
            return
        ...
        for detector in detectors_to_merge:
            ...
            for i in range(len(detector.frames)):
                ...
                for attr in _FLUX_ARRAY_ATTRS:
                    all_flux_values[attr].append(
                        self._optional_detection_value(detector, attr, i)
                    )
    ...
    def _on_undo_description_changed(self, description: str) -> None:
        ...
        self.undo_btn.setToolTip(
            description if description else "Undo last detection operation"
        )
