...
class ImageryViewer(QWidget):
    ...
    def __init__(self):
        ...
        self.temp_track_plot = (
            None  # Temporary plot item for track being created/edited
        )
        ...
        self.temp_detection_plot = (
            None  # Temporary plot item for detections being created/edited
        )
        ...
        self.selected_detections_plot = (
            None  # ScatterPlotItem for highlighting selected detections
        )

        # Extraction view mode
        self.extraction_view_mode = False
        self.viewing_extraction_track = (
            None  # Track object whose extraction is being viewed
        )
        self.extraction_overlay = (
            None  # ImageItem for displaying extraction signal pixels
        )
        ...
        self.extraction_painting = (
            False  # True when mouse is held down for drag painting
        )
        ...
        self.ewma_frame_counter = (
            0  # Number of unique frames viewed since filter enabled
        )
        self.ewma_last_frame_number = (
            None  # Last frame_number passed to set_frame_number
        )
    ...
    def select_imagery(self, imagery: Imagery, preserve_ewma: bool = False):
        ...
        if imagery in self.imageries:
            ...
            if (
                ...
                and frame_index >= imagery.loaded_frame_count
            ):
    ...
    def set_frame_number(self, frame_number: int):
        ...
        if self.imagery is not None and len(self.imagery.frames) > 0:
            ...
            if image_index is not None:
                ...
                if self.ewma_filter_enabled:
                    ewma_alpha = self.settings.value(
                        "toolbar/ewma_decay_factor", 0.1, type=float
                    )
                    ewma_offset = self.settings.value(
                        "toolbar/ewma_frame_offset", 1, type=int
                    )
                    ...
                    if (self.ewma_background is not None) and (
                        self.ewma_frame_counter > ewma_offset
                    ):
                ...
                if ewma_active:
                    min_percentile = self.settings.value(
                        "imagery/histogram_min_percentile", 1.0, type=float
                    )
                    max_percentile = self.settings.value(
                        "imagery/histogram_max_percentile", 99.0, type=float
                    )
                    settings_bins = self.settings.value(
                        "imagery/histogram_bins", 256, type=int
                    )
                    settings_max_rowcol = self.settings.value(
                        "imagery/histogram_max_rowcol", 512, type=int
                    )

                    row_downsample = max(
                        1, self.ewma_image.shape[0] // settings_max_rowcol
                    )
                    col_downsample = max(
                        1, self.ewma_image.shape[1] // settings_max_rowcol
                    )
                    downsampled_display_image = self.ewma_image[
                        ::row_downsample, ::col_downsample
                    ]

                    # Remove zero values since there are often many of these values
                    nonzero_downsampled_display_image = downsampled_display_image[
                        downsampled_display_image != 0
                    ]

                    # Compute data range
                    if user_histogram_bounds is None:
                        if nonzero_downsampled_display_image.size > 0:
                            hist_min = np.percentile(
                                nonzero_downsampled_display_image, min_percentile
                            )
                            hist_max = np.percentile(
                                nonzero_downsampled_display_image, max_percentile
                            )
                        else:
                            hist_min = -1.0
                            hist_max = 1.0
                    ...
                    hist_y, bin_edges = np.histogram(
                        nonzero_downsampled_display_image, bins=settings_bins
                    )
                    ...
                    if self.histogram_visible:
                        self.histogram.plot.setData(
                            bin_centers[nonzero_hist], hist_y[nonzero_hist]
                        )
        ...
        if self.last_mouse_pos is not None and (
            self.geolocation_enabled or self.pixel_value_enabled
        ):
        ...
        if ENABLE_PERF_MONITORING and perf_start is not None:
            ...
            if len(self.perf_frame_times) >= 60:
                ...
                print(
                    f"[PERF] Frame update: {avg_time * 1000:.2f}ms avg, {fps:.1f} FPS (last 60 frames)"
                )
    ...
    def get_current_time(self):
        ...
        if (
            self.imagery is not None
            and self.imagery.times is not None
            and len(self.imagery.frames) > 0
        ):
            # Try exact frame match first using optimized lookup
            ...
            if image_index is None:
                valid_indices = np.where(
                    self.imagery.frames <= self.current_frame_number
                )[0]
                if len(valid_indices) > 0:
                    image_index = valid_indices[-1]
                else:
    ...
    def on_histogram_levels_changed(self):
        ...
        if (
            self.ignore_changing_histogram_levels
            or self.setting_imagery
            or (self.imagery is None)
        ):
            return
    ...
    def update_text_positions(self):
        ...
        if self.map_view_mode:
            ...
            offset_sign = (
                -1
            )  # offset upward in data coords = upward on screen (Y inverted)
        ...
        if self.geolocation_enabled and self.geolocation_text.isVisible():
            ...
            if self.pixel_value_enabled and self.pixel_value_text.isVisible():
                ...
                self.geolocation_text.setPos(
                    view_rect.right(), bottom_y + offset_sign * text_offset
                )
            else:
    ...
    def update_overlays(self):
        ...
        orphaned_detector_uuids = (
            set(self.detector_plot_items.keys()) - current_detector_uuids
        )
        ...
        for detector in self.detectors:
            ...
            if (
                self.selected_sensor is not None
                and detector.sensor != self.selected_sensor
            ):
            ...
            if np.any(det_mask):
                ...
                _det_map_mode = (
                    self.map_view_mode
                    and detector.sensor
                    and detector.sensor.can_geolocate()
                )
        ...
        for track in self.tracks:
            ...
            if (
                self.selected_sensor is not None
                and track.sensor != self.selected_sensor
            ):
            ...
            _map_mode = (
                self.map_view_mode and track.sensor and track.sensor.can_geolocate()
            )
            ...
            if track.complete:
                ...
                if visible_indices is not None and len(visible_indices) > 0:
                    ...
                    if _map_mode:
                        path_x, path_y = (
                            geodetic[0][visible_indices],
                            geodetic[1][visible_indices],
                        )
                    else:
                        path_x, path_y = (
                            track.columns[visible_indices],
                            track.rows[visible_indices],
                        )
            ...
            if (
                ...
                if track_data is not None:
                    ...
                    uncertainty_style = settings.value(
                        "tracks/uncertainty_line_style", "DashLine", type=str
                    )
                    uncertainty_width = settings.value(
                        "tracks/uncertainty_line_width", 1, type=int
                    )
                    uncertainty_scale = settings.value(
                        "tracks/uncertainty_scale", 1.0, type=float
                    )
                    ...
                    if idx is not None:
                        ...
                        if ellipse_params is not None:
                            ...
                            pen = track.get_pen(
                                width=uncertainty_width, style=uncertainty_style
                            )
                            ellipse.setPen(pen)
    ...
    def add_tracker(self, tracker):
        ...
        warnings.warn(
            "add_tracker() is deprecated. Use add_tracks() instead.",
            DeprecationWarning,
            stacklevel=2,
        )
    ...
    def filter_by_sensor(self, sensor):
        ...
        if sensor is not None:
            ...
            if self.imagery is None or self.imagery.sensor != sensor:
                ...
                sensor_imageries = [
                    img for img in self.imageries if img.sensor == sensor
                ]
                if sensor_imageries:
    ...
    def _complete_lasso_selection(self):
        ...
        for track in self.tracks:
            ...
            for x, y in zip(check_x, check_y):
                if (
                    np.isnan(x)
                    or np.isnan(y)
                    or not lasso_polygon.contains(Point(x, y))
                ):
                    all_contained = False
                    break
        ...
        for detector in self.detectors:
            ...
            for i, (x, y) in enumerate(zip(check_x, check_y)):
                if (
                    not np.isnan(x)
                    and not np.isnan(y)
                    and lasso_polygon.contains(Point(x, y))
                ):
                    # Store as (detector, frame, original_index)
                    detection_frame = detector.frames[indices[i]]
                    selected_items["detections"].append(
                        (detector, int(detection_frame), int(indices[i]))
                    )
        ...
        if not _map_mode:
            for aoi in self.aois:
                ...
                all_contained = all(
                    lasso_polygon.contains(Point(corner)) for corner in corners
                )
                if all_contained:
            ...
            if isinstance(feature, PlacemarkFeature):
                ...
                if _map_mode:
                    ...
                    if (
                        ...
                        and self.selected_sensor.can_geolocate()
                    ):
    ...
    def on_mouse_moved(self, pos):
        ...
        if self.extraction_editing_mode and self.pan_zoom_locked:
            ...
            if QApplication.mouseButtons() & Qt.MouseButton.LeftButton:
                ...
                if (
                    self.extraction_editor is not None
                    and self.editing_extraction_track is not None
                ):
                    ...
                    if self.plot_item.sceneBoundingRect().contains(pos):
                        ...
                        # Get chip position
                        chip_position = (
                            self.extraction_editor.get_current_chip_position()
                        )
                        if chip_position is not None:
                            chip_top, chip_left = chip_position
                            chip_size = self.extraction_editor.working_extraction[
                                "chip_size"
                            ]
                            ...
                            if 0 <= chip_row < chip_size and 0 <= chip_col < chip_size:
                                ...
                                self.extraction_editor.paint_pixel(
                                    chip_row, chip_col, is_drag=True
                                )
    ...
    def _update_tooltips_at_position(self, pos):
        ...
        if (
            not (self.geolocation_enabled or self.pixel_value_enabled)
            or self.imagery is None
        ):
            return
        ...
        if self.map_view_mode:
            ...
            if self.pixel_value_enabled and self.imagery.sensor.can_geolocate():
                ...
                if len(valid_indices) > 0:
                    ...
                    src_rows, src_cols = self.imagery.sensor.geodetic_to_pixel(
                        frame, earth_loc
                    )
        ...
        if self.imagery.images is not None and len(self.imagery.images) > 0:
            img_shape = self.imagery.images[0].shape
            if (
                0 <= imagery_relative_row < img_shape[0]
                and 0 <= imagery_relative_col < img_shape[1]
            ):
                ...
                if len(valid_indices) > 0:
                    ...
                    if self.geolocation_enabled and self.imagery.sensor.can_geolocate():
                        ...
                        locations = self.imagery.sensor.pixel_to_geodetic(
                            frame, rows_array, cols_array
                        )
                    ...
                    if self.pixel_value_enabled:
                        ...
                        pixel_value = self.imagery.images[
                            image_index, row_floor, col_floor
                        ]
    ...
    def start_draw_roi(self):
        ...
        img_shape = (
            self.imagery.images[0].shape if len(self.imagery.images) > 0 else (100, 100)
        )
    ...
    def snap_roi_to_integers(self, roi):
        ...
        if (
            new_pos[0] != pos.x()
            or new_pos[1] != pos.y()
            or new_size[0] != size.x()
            or new_size[1] != size.y()
        ):
    ...
    def _render_placemark(self, feature):
        ...
        if (
            self.map_view_mode
            and geometry.get("lon") is not None
            and geometry.get("lat") is not None
        ):
            x_pos = geometry["lon"]
            y_pos = geometry["lat"]
        else:
    ...
    def _render_shapefile(self, feature):
        ...
        if (
            not self.map_view_mode
            and self.selected_sensor is not None
            and self.selected_sensor.can_geolocate()
        ):
    ...
    def finish_track_editing(self):
        ...
        if editing_track and len(self.current_track_data) > 0:
            ...
            editing_track.rows = np.array(
                [self.current_track_data[f][0] for f in sorted_frames]
            )
            editing_track.columns = np.array(
                [self.current_track_data[f][1] for f in sorted_frames]
            )
    ...
    def start_extraction_editing(self, track, imagery):
        ...
        if self.extraction_editor is None:
            ...
            self.extraction_editor.frame_changed.connect(
                self.on_extraction_editor_frame_changed
            )
            self.extraction_editor.signal_mask_updated.connect(
                self._update_extraction_overlay_from_editor
            )
            self.extraction_editor.extraction_saved.connect(self.on_extraction_saved)
            self.extraction_editor.extraction_cancelled.connect(
                self.on_extraction_cancelled
            )
            self.extraction_editor.lock_pan_zoom_changed.connect(
                self.on_lock_pan_zoom_changed
            )
    ...
    def _update_extraction_overlay_from_editor(self):
        ...
        # Update overlay image
        self.extraction_overlay.setImage(overlay, autoLevels=False)
        self.extraction_overlay.setPos(
            chip_position[1], chip_position[0]
        )  # (left, top)
    ...
    def start_detection_editing(self, detector):
        ...
        for i in range(len(detector.frames)):
            ...
            self.current_detection_data[frame].append(
                (detector.rows[i], detector.columns[i])
            )
        self.temp_detection_plot = None
        # Update cursor based on all interactive modes
        self.update_cursor()
    ...
    def _refine_clicked_point(self, row, col):
        ...
        mode = params.pop(
            "mode"
        )  # Remove mode from params to avoid duplicate keyword argument
        ...
        if self.plot_item.sceneBoundingRect().contains(pos):
            ...
            if self.track_creation_mode or self.track_editing_mode:
                ...
                if self.current_frame_number in self.current_track_data:
                    existing_row, existing_col = self.current_track_data[
                    ...
                    if self.map_view_mode:
                        ...
                        distance = np.sqrt(
                            (col - existing_col) ** 2 + (row - existing_row) ** 2
                        )
                ...
                refined_row, refined_col = self._refine_clicked_point(
                    pixel_row, pixel_col
                )

                # Store in pixel coordinates (tracks always store row/col)
                self.current_track_data[self.current_frame_number] = (
                    refined_row,
                    refined_col,
                )
            ...
            # Handle detection creation/editing
                ...
                self.current_detection_data[self.current_frame_number].append(
                    (refined_row, refined_col)
                )
            ...
            # Handle extraction editing
                ...
                if (
                    self.extraction_editor is None
                    or self.editing_extraction_track is None
                ):
                    return
            ...
            # Handle track selection
                ...
                for track in self.tracks:
                    ...
                    if track.complete:
                        ...
                        frame_mask = track.frames >= (
                            self.current_frame_number - track.tail_length
                        )
                        frame_mask &= track.frames <= self.current_frame_number
                    else:
                    ...
                    if self.map_view_mode:
                        ...
                        if geodetic is not None:
                            track_x, track_y = (
                                geodetic[0][frame_mask],
                                geodetic[1][frame_mask],
                            )
                        else:
                        ...
                        distances = np.sqrt(
                            (visible_cols - col) ** 2 + (visible_rows - row) ** 2
                        )
                            ...
                            if (
                                ...
                                if hasattr(self.data_manager, "detections_panel"):
                                    ...
                                    if detector.complete:
                                        ...
                                        # For current frame mode, combine frame and label filters
                                        frame_mask = (
                                            detector.frames == self.current_frame_number
                                        )
                                        combined_mask = frame_mask & label_mask
                                        if np.any(combined_mask):
                                            rows = detector.rows[combined_mask]
    ...
    def set_map_view_mode(self, enabled: bool) -> bool:
        ...
        if enabled:
            ...
            coarse_size = (
                settings.value("wms/coarse_grid_size", 64, type=int)
                if coarse_enabled
                else 0
            )
            self.imagery_projector = ImageryProjector(
                self.selected_sensor, coarse_grid_size=coarse_size
            )
            ...
            # Block signals to prevent histogram recomputation
            try:
                self.projected_image_item.sigImageChanged.disconnect(
            ...
            try:
                self.plot_item.vb.sigRangeChanged.disconnect(
                    self._on_map_view_range_changed
                )
            except (TypeError, RuntimeError):
                pass
            ...
            if self.wms_fetcher_thread is not None:
                ...
                try:
                    self.wms_fetcher_thread.tile_fetched.disconnect(
                        self._on_wms_tile_fetched
                    )
                    self.wms_fetcher_thread.all_tiles_fetched.disconnect(
                        self._on_all_wms_tiles_fetched
                    )
                    self.wms_fetcher_thread.error_occurred.disconnect(
                        self._on_wms_error
                    )
                except (TypeError, RuntimeError):
                    pass
    ...
    def fit_prf_for_sensor(self, sensor=None, settings: QSettings | dict | None = None):
        ...
        imagery = (
            self.imagery
            if self.imagery is not None and self.imagery.sensor == sensor
            else None
        )
        if imagery is None:
            imagery = next(
                (
                    candidate
                    for candidate in self.imageries
        ...
        model = self._prf_setting(
            settings, "imagery/prf_model", "Elliptical Gaussian", str
        )
        if model == NO_PRF_MODEL:
            model = "Elliptical Gaussian"

        min_detections = self._prf_setting(
            settings, "imagery/prf_min_detections", 5, int
        )
        chip_size = self._prf_setting(settings, "imagery/prf_chip_size", 11, int)
        if chip_size % 2 == 0:
            chip_size += 1
        pixel_shape = self._prf_setting(
            settings, "imagery/prf_pixel_shape", "Square", str
        )
        tolerance = self._prf_setting(settings, "imagery/prf_tolerance", 0.01, float)
        max_iterations = self._prf_setting(
            settings, "imagery/prf_max_iterations", 50, int
        )
        fit_max_detections = self._prf_setting(
            settings, "imagery/prf_fit_max_detections", 40, int
        )
        fit_max_detections = max(int(fit_max_detections), int(min_detections))
        adaptive_fit = self._prf_setting(
            settings, "imagery/prf_adaptive_fit", True, bool
        )
        ...
        detection_source = normalize_prf_fit_detection_source(
            self._prf_setting(
                settings,
                "imagery/prf_detection_source",
                "Selected detection chips",
                str,
            )
        )
        auto_max_detections = self._prf_setting(
            settings, "imagery/prf_auto_max_detections", 150, int
        )

        candidates = self._prf_detection_candidates_for_imagery(
            imagery, detection_source
        )
        ...
        prf_model = fit_prf_model_adaptive(
            chips=chips,
            model=model,
        ...
        fitted_prf_metadata = {
            ...
            "adaptive_fit": bool(adaptive_fit),
    ...
    def _prf_detection_candidates_for_imagery(
        self, imagery, detection_source: str
    ) -> list[tuple]:
    ...
    def _fetch_wms_tiles(self) -> None:
        ...
        # Compute tile grid
        tile_coords, zoom_level = self.wms_client.compute_tile_grid(
            view_bbox, view_width_px
        )
        if not tile_coords:
            return
        ...
        self.wms_status_changed.emit(
            f"Loading map tiles (0/{self._wms_total_tiles})..."
        )
        ...
        self.wms_fetcher_thread.all_tiles_fetched.connect(
            self._on_all_wms_tiles_fetched
        )
    ...
    def _on_wms_tile_fetched(self, tile) -> None:
        ...
        self.wms_status_changed.emit(
            f"Loading map tiles ({loaded}/{self._wms_total_tiles})..."
        )
    ...
    def _on_all_wms_tiles_fetched(self) -> None:
        ...
        for tile in self._pending_tiles:
            ...
            if is_geographic:
                ...
                if tile_flipped.shape[1] != dst_w or tile_flipped.shape[0] != dst_h:
                    ...
                    pil_resized = pil_img.resize(
                        (dst_w, dst_h), PILImage.Resampling.BILINEAR
                    )
            ...
            if cw > 0 and ch > 0:
                composite[cy : cy + ch, cx : cx + cw] = resized_arr[
                    sy : sy + ch, sx : sx + cw
                ]
    ...
    def _update_projected_imagery(self, image=None) -> None:
        ...
        if (
            not self.map_view_mode
            or self.imagery is None
            or self.imagery_projector is None
        ):
            return
        ...
        image_index = self.imagery.get_frame_index(self.current_frame_number)
        if image_index is None:
            valid_indices = np.where(self.imagery.frames <= self.current_frame_number)[
                0
            ]
            if len(valid_indices) > 0:
                image_index = valid_indices[-1]
        ...
        frame = self.imagery.frames[image_index]
        zoom_level = (
            self._current_zoom_level if self._current_zoom_level is not None else 0
        )
